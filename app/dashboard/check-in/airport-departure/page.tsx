...
type Item = {
  ...
  departureFlightDetail?: string;
  departureCheckInStatus?: boolean;
  departureCheckInTime?: string;
};
...
export default function AirportDeparturePage() {
  ...
  const openConfirm = (item: Item) => {
    if (item.departureCheckInStatus) return;
    setSelected(item);
    setShowConfirm(true);
  };
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${API}/api/checkin-details/${selected._id}/departure`,
        {
          method: 'PUT',
          credentials: 'include',
          headers: {
...
<div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-4">
  <div className="max-w-6xl mx-auto">
    ...
    <div className="mb-6">
      ...
      <h1 className="text-2xl font-bold mt-3">Faculty Airport Departure</h1>
    </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      {paginated.map((m) => {
        const checked = !!m.departureCheckInStatus;
        const displayTime = m.departureCheckInTime
          ? new Date(m.departureCheckInTime).toLocaleString()
          : '-';
        ...
        return (
          ...
          >
            <CardContent>
              ...
              {/* DEPARTURE INFO */}
              <div className="space-y-2 mb-4 p-3 bg-green-50 rounded-lg text-sm">
                <div className="flex items-center gap-2">
                  <Calendar className="w-4 h-4 text-green-600" />
                  <strong>Departure Date:</strong> {m.departureDate}
                </div>

                <div className="flex items-center gap-2">
                  <Clock className="w-4 h-4 text-green-600" />
                  <strong>Departure Time:</strong> {m.departureTime}
                </div>

                <div className="flex items-center gap-2">
                  <Plane className="w-4 h-4 text-green-600" />
                  <strong>Flight:</strong> {m.departureFlightDetail}
                </div>
