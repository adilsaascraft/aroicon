...
export default function HotelPage() {
  ...
  const openConfirm = (m: Item) => {
    if (m.hotelCheckInStatus) return;
    setSelected(m);
    setShowConfirm(true);
  };
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/api/checkin-details/${selected._id}/hotel`, {
        method: 'PUT',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
  ...
  <div className="max-w-6xl mx-auto">
    ...
    <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
      <div className="relative max-w-md">
        <HotelIcon className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 w-4 h-4" />
        <Input
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      {paginated.map((m) => {
        ...
        const displayTime = m.hotelCheckInTime
        ...
        return (
          ...
          >
            <CardContent>
              ...
              {/* HOTEL INFO */}
              <div className="space-y-2 mb-4 p-3 bg-yellow-50 rounded-lg text-sm">
